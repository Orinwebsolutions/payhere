...
//		global $wpdb;
//		$transaction_db_table_name = $wpdb->prefix.'transactions';
//		$wpdb->query($wpdb->prepare("UPDATE $transaction_db_table_name SET status=1 , paypal_transection_id ='".$payment_id."' where trans_id = '".$_REQUEST['trans_id']."'"));
//		$wpdb->query($wpdb->prepare("UPDATE $wpdb->posts SET post_status='".fetch_posts_default_paid_status()."' where ID = %d",wp_kses_post($_REQUEST['pid'])));
                
//                $wpdb->query($wpdb->prepare("UPDATE $transaction_db_table_name SET status=1 , paypal_transection_id =%d where trans_id = %d",$payment_id,$order_id));
